                ...
                <div className="flex h-16 justify-between">
                    <div className="flex">
                        ...
                        <div className="hidden xl:ml-6 xl:flex xl:space-x-8">
                            {staticNavigation.map((item) => {
                                const isActive = isLinkActive(item.href) || (item.children && item.children.some(child => isLinkActive(child.href)));
                    ...
                    <div className="flex items-center gap-2">
                        <div className="hidden xl:flex xl:items-center xl:gap-4 xl:ml-2">
                            {/* ... social icons ... */}
                            {config?.facebookUrl && (
                                <Link href={ensureProtocol(config.facebookUrl)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition transform hover:scale-110">
                                    <Facebook className="h-5 w-5" />
                                </Link>
                            )}
                            {config?.instagramUrl && (
                                <Link href={ensureProtocol(config.instagramUrl)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 transition transform hover:scale-110">
                                    <Instagram className="h-5 w-5" />
                                </Link>
                            )}
                            {config?.tiktokUrl && (
                                <Link href={ensureProtocol(config.tiktokUrl)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-black dark:hover:text-white transition transform hover:scale-110">
                            ...
                            {config?.discordUrl && (
                                <Link href={ensureProtocol(config.discordUrl)} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition transform hover:scale-110">
                                    ...
                                    <div className={cn("mt-2 pl-4 space-y-2 border-l-2 border-indigo-100 dark:border-gray-800", hoveredItem === item.name ? "block" : "hidden")}>
                                        <Link
                                            ...
                                            className={cn(
                                                ...
                                                isLinkActive(item.href) ? "text-indigo-600 dark:text-indigo-400" : "text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"
                                            )}
                                        ...
                                        {item.children.map((child) => (
                                            <Link
                                                ...
                                                className={cn(
                                                    ...
                                                    isLinkActive(child.href) ? "text-indigo-600 dark:text-indigo-400" : "text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"
                                                )}
                                                onClick={() => setIsOpen(false)}
                                            >
                                                {child.name}
